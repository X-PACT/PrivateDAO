...
| Confidential Treasury Command Center | Live guided UI path that turns `Create -> Commit -> Reveal -> Execute` into one product flow, then explains proposal-by-proposal whether ZK, REFHE, MagicBlock, and backend-indexed RPC are active, optional, or not required. | [`docs/index.html`](docs/index.html), [`docs/frontier-guided-flow.md`](docs/frontier-guided-flow.md) |
