...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      explanation:
        "Bid amounts, bidder labels, salts, and auction momentum are not returned. The legacy package verifies a hash commitment; use /api/auctions/sealed/outcome-proof for Groth16 outcome verification.",
    });
  } catch (error) {
    return NextResponse.json(
      {
