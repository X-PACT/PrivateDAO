...
export async function POST(request: Request) {
  ...
  const result = await provider.treasuryContext(sanitizeIntelligenceRequest({ ...body, includeTreasuryContext: true }));
  return NextResponse.json({ ok: true, result, dataSentToProviders: body.treasuryAddress ? ["treasury address"] : [] });
}
