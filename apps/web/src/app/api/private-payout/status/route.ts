...
export async function GET() {
  ...
  const active = providers.find((provider) => provider.provider === "umbra" && provider.configured) ?? providers.find((provider) => provider.provider === "mock-testnet");
  return NextResponse.json({
    ok: true,
    activeProvider: active?.provider ?? "mock-testnet",
