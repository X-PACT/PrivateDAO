...
export default function SecurityPage() {
  return (
    <OperationsShell
      eyebrow="Security"
      title="Security for private decisions and public verification."
      description="PrivateDAO is designed so organizations can use sensitive data in workflows, keep private values out of public proof pages, and still give auditors a verification trail they can inspect."
      navigationMode="guided"
      badges={[
        { label: "Encryption", variant: "violet" },
        { label: "Audit trail", variant: "success" },
        { label: "Verification model", variant: "cyan" },
      ]}
    >
      <section className="rounded-[28px] border border-emerald-300/16 bg-emerald-300/[0.06] p-5 sm:p-6">
        <div className="text-[11px] uppercase tracking-[0.25em] text-emerald-100/76">Institutional security model</div>
        <h2 className="mt-3 text-2xl font-semibold text-white">Keep private data protected while making the outcome verifiable.</h2>
        <p className="mt-3 max-w-4xl text-sm leading-7 text-white/64">
          PrivateDAO separates private operating data from public verification. Earnings, reviewer notes, votes,
          thresholds, and internal policies can be used in a workflow without being exposed in the public proof package.
        </p>
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {[
            ["Encryption", "Sensitive values are handled as private workflow inputs and excluded from public verification pages."],
            ["Data isolation", "Product records, proof records, and provider data are separated by workflow, organization, and disclosure boundary."],
            ["Audit trail", "Every important workflow step can produce a timestamped proof event and reviewable public outcome."],
            ["Verification model", "Third parties recompute the public proof package hash and detect tampering without seeing private data."],
            ["Infrastructure", "AWS hosts the product/API layer, Supabase stores operational records, and Solana provides a public proof anchor where applicable."],
            ["Commercial deployment", "Cloud SaaS and private deployment paths support different buyer requirements without changing the verification promise."],
          ].map(([title, body]) => (
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link href="/proof-workflows/verify/demo-proof-id" className="inline-flex items-center text-sm font-semibold text-emerald-100 hover:text-white">
      ...
      <LiveWorkflowExplainer variant="security" compact />
      <div>
