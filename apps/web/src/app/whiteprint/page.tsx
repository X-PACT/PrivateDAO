...
const corePrinciples = [
  ...
  ["Composability", "PrivateDAO should integrate naturally with the networks and systems each product genuinely needs."],
] as const;
...
const architectureLayers = [
  ...
  ["Execution Layer", "Integration with treasury systems, evidence storage, and certified network providers."],
  ["Kernel Layer", "A frozen lifecycle boundary for product invocations, policy, authorization, proof, replay protection, and evidence."],
] as const;
...
const initialUseCases = [
  ...
  ["Confidential Payroll", "Prepare payroll privately, apply policy, settle, and share a proof without exposing individual data."],
  ["Private Auctions", "Run sealed bids and publish a result that participants can verify."],
  ["Agent Marketplace", "Let software discover services, invoke providers, pay, and verify receipts."],
  ["PDAO Worlds", "Use a game world to make privacy, proof, and coordination understandable to a wider audience."],
  ...
  return (
    <OperationsShell
      ...
      badges={[
        ...
        { label: "Four commercial paths", variant: "violet" },
      ]}
    >
      ...
      <section className="rounded-[28px] border border-cyan-300/16 bg-cyan-300/[0.06] p-5 sm:p-6">
        ...
        <div className="mt-4 max-w-5xl space-y-3 text-sm leading-7 text-white/66">
          ...
          <p>
            Governance becomes visible. Operations remain invisible. PrivateDAO is building a confidential coordination
            platform that starts with a clear customer outcome and stays ready for multiple networks.
          </p>
          <p>
            The objective is not simply enabling voting. It is helping organizations verify records, govern privately,
            coordinate treasury work, and produce evidence without exposing unnecessary operational details.
          </p>
        </div>
      ...
      <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5 sm:p-6">
        <div className="text-[11px] uppercase tracking-[0.25em] text-white/46">Commercial product map</div>
        <h2 className="mt-3 text-2xl font-semibold text-white">Clear products for real operating work.</h2>
        <p className="mt-3 max-w-4xl text-sm leading-7 text-white/64">
          Customers choose the outcome first: run payroll, coordinate treasury, govern privately, run sealed auctions, verify records, or connect an agent marketplace. Integrations support these products rather than becoming separate products.
        </p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {commercialProductGroups.map((group) => <div key={group.title} className="rounded-2xl border border-white/10 bg-black/20 p-4">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <article className="rounded-[28px] border border-violet-300/16 bg-violet-300/[0.06] p-5 sm:p-6">
          ...
          <p className="mt-3 text-sm leading-7 text-white/64">
            PrivateDAO starts with Solana where the current execution and anchoring paths are strongest. Cross-network
            expansion is capability-led: a network is presented as supported only after its provider, wallet, finality,
            and end-to-end behavior are implemented and tested for the relevant product.
          </p>
          <div className="mt-5">
      ...
      <section className="rounded-[30px] border border-amber-300/18 bg-amber-300/[0.06] p-5 sm:p-7">
        <div className="text-[11px] uppercase tracking-[0.25em] text-amber-100/76">Kernel and network plan</div>
        <h2 className="mt-3 max-w-5xl text-2xl font-semibold text-white">One product language, many execution options.</h2>
        <p className="mt-3 max-w-5xl text-sm leading-7 text-white/66">
          The PrivateDAO Kernel is the shared foundation, not a customer-facing product. Products call generic capabilities
          for identity, policy, proof, evidence, storage, signing, execution, and finality. Network providers live outside
          the product logic, so a new network can be added without rewriting the customer workflow.
        </p>
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {[
            ["Now", "Verify and core evidence flows on the currently tested Solana path."],
            ["Next", "Complete Govern and Coordinate on the same Kernel-bound application core."],
            ["Expand", "Add EVM and Stellar capabilities where product requirements justify them."],
            ["Later", "Evaluate Bitcoin, Starknet, and Polkadot for anchoring, execution, or interoperability use cases."],
          ].map(([title, body]) => (
