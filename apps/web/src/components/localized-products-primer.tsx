...
export function LocalizedProductsPrimer() {
  const { copy } = useI18n();
  const content = copy.pageContent.productsPrimer;
    ...
    <section className="rounded-[30px] border border-white/10 bg-white/[0.04] p-6">
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <Link href="/services" className={cn(buttonVariants({ size: "sm" }))}>
          {content.openServices}
        </Link>
        <Link href="/services/privacy-sdk-api-starter" className={cn(buttonVariants({ size: "sm", variant: "secondary" }))}>
          {content.openSdkStarter}
        </Link>
        <Link href="/trust" className={cn(buttonVariants({ size: "sm", variant: "secondary" }))}>
          {content.openTrustRoute}
        </Link>
      </div>
    </section>
  );
