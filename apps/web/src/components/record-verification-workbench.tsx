...
type RecordReceipt = { receipt_id: string; record_type: string; record_id: string; canonical_record_digest: string; anchor_network: string; anchor_reference: string | null; anchor_status?: string; anchor_signature?: string | null; anchor_cluster?: string | null; anchor_slot?: number | null; public_fields: Record<string, unknown>; verification_status: string };
...
export function RecordVerificationWorkbench() {
  ...
  async function verify() {
    ...
    try {
      ...
      setReceipt(body.receipt); setStatus("verified");
  ...
  return <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
    <section className="rounded-[28px] border border-white/10 bg-black/20 p-5 sm:p-7">
      ...
      <label className="mt-5 block text-sm font-medium text-white" htmlFor="record-json">Record JSON</label>
      <textarea id="record-json" value={raw} onChange={(event) => setRaw(event.target.value)} rows={16} spellCheck={false} className="mt-2 w-full rounded-2xl border border-white/10 bg-black/35 p-4 font-mono text-xs leading-6 text-white outline-none focus:border-cyan-200/50" />
    ...
    <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5 sm:p-7" aria-live="polite">
      ...
      {receipt && <><div className="mt-5 flex items-center gap-3 text-emerald-100"><CheckCircle2 className="h-7 w-7" /><span className="text-2xl font-semibold">VERIFIED</span></div><dl className="mt-6 grid gap-4 text-sm"><div><dt className="text-white/45">Record</dt><dd className="mt-1 text-white">{receipt.record_type} / {receipt.record_id}</dd></div><div><dt className="text-white/45">Canonical digest</dt><dd className="mt-1 break-all font-mono text-xs text-cyan-100">{receipt.canonical_record_digest}</dd></div><div><dt className="text-white/45">Solana anchor</dt><dd className="mt-1 text-white">{receipt.anchor_status === "confirmed" ? `${receipt.anchor_cluster || "Solana"} confirmed` : receipt.anchor_status || "Pending anchor configuration"}</dd>{receipt.anchor_signature && <dd className="mt-1 break-all font-mono text-xs text-cyan-100">{receipt.anchor_signature}</dd>}</div></dl><div className="mt-6 rounded-2xl border border-white/10 bg-black/20 p-4"><div className="text-sm font-medium text-white">Public fields</div><pre className="mt-3 overflow-auto text-xs leading-6 text-white/68">{JSON.stringify(receipt.public_fields, null, 2)}</pre></div><button type="button" onClick={copyReceipt} className="mt-5 inline-flex items-center gap-2 rounded-xl border border-white/15 px-4 py-2 text-sm text-white hover:bg-white/5"><Copy className="h-4 w-4" />{copied ? "Copied" : "Copy public receipt link"}</button></>}
    </section>
  </div>;
}
