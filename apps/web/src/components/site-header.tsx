"use client";

import Link from "next/link";
import { LockKeyhole, Search, Sparkles } from "lucide-react";

import { LanguageSwitcher } from "@/components/language-switcher";
import { WalletConnectButton } from "@/components/wallet-connect-button";
import { useI18n } from "@/components/i18n-provider";
import { buttonVariants } from "@/components/ui/button";
import { useSiteUrls } from "@/lib/site-urls";
import { cn } from "@/lib/utils";

const navItems = [
  { href: "/products", label: "Products" },
  { href: "/payroll", label: "Payroll" },
  { href: "/treasury", label: "Treasury" },
  { href: "/govern", label: "Governance" },
  { href: "/auctions", label: "Auctions" },
  { href: "/agents", label: "Agents" },
];

const utilityNav: Array<{ href: string; label: string; rel?: string }> = [
  { href: "/developers", label: "Developers" },
  { href: "/whitepaper", label: "Whitepaper" },
  { href: "/thesis", label: "Thesis" },
  { href: "/contact", label: "Contact" },
];

export function SiteHeader() {
  const { liveSiteUrl } = useSiteUrls();
  const { copy } = useI18n();
  const resolvePrimaryLabel = (href: string, fallback: string) => {
    switch (href) {
      case "/start":
      case "/govern#live-dao":
      case "/try":
        return copy.chrome.start;
      case "/rooms":
        return "Rooms";
      case "/learn":
        return copy.chrome.learn;
      case "/govern":
        return "Govern";
      case "/intelligence":
        return "Intelligence";
      case "/treasury":
        return "Treasury";
      case "/proof-workflows":
        return "Workflows";
      case "/gaming":
        return "GamingDAO";
      case "/android":
        return "Android";
      case "/proof":
        return "Proof";
      case "/judge":
        return "Judge";
      default:
        return fallback;
    }
  };
  const resolveUtilityLabel = (href: string, fallback: string) => {
    switch (href) {
      case "/rpc-services":
        return "RPC";
      case "/api-status":
        return "API Status";
      case "/services":
        return copy.chrome.apiPricing;
      case "/documents":
        return copy.chrome.docs;
      case "/whitepaper":
        return "Whitepaper";
      case "/search":
        return copy.chrome.search;
      default:
        return fallback;
    }
  };
  return (
    <header className="sticky top-0 z-40 border-b border-[#dce5f0] bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4 lg:items-center">
          <div className="group flex min-w-0 flex-col gap-0">
            <Link href="/" className="min-w-0">
              <div className="flex flex-nowrap items-center gap-0.5 whitespace-nowrap text-lg font-semibold tracking-tight text-[#10233f] sm:text-2xl">
                <span>PrivateD</span>
                <span className="inline-block bg-[linear-gradient(135deg,#175cd3,#6c3fd1)] bg-clip-text text-[1.3rem] font-black text-transparent sm:text-[1.7rem]">
                  △
                </span>
                <span>O</span>
              </div>
              <div className="mt-1 hidden items-center gap-2 text-[11px] uppercase tracking-[0.24em] text-[#5d6d82] lg:flex">
                <LockKeyhole className="h-3.5 w-3.5 text-[#175cd3]" />
                <span>{copy.chrome.createPrivateDaoTagline}</span>
              </div>
            </Link>
          </div>

          <div className="flex w-full shrink-0 flex-wrap items-center gap-1.5 sm:w-auto sm:flex-nowrap sm:gap-2">
            <LanguageSwitcher />
            <Link
              href="/search"
              className={cn(buttonVariants({ size: "sm", variant: "ghost" }), "hidden h-10 w-10 rounded-full p-0 text-[#5d6d82] sm:inline-flex")}
              aria-label={copy.chrome.search}
            >
              <Search className="h-4 w-4" />
            </Link>
            <Link
              href="/assistant"
              className={cn(buttonVariants({ size: "sm", variant: "ghost" }), "hidden h-10 w-10 rounded-full p-0 text-[#5d6d82] sm:inline-flex")}
              aria-label={copy.chrome.help}
            >
              <Sparkles className="h-4 w-4" />
            </Link>
            <a
              className={cn(buttonVariants({ size: "sm", variant: "secondary" }), "hidden xl:inline-flex")}
              href={liveSiteUrl}
              rel="noreferrer"
              target="_blank"
            >
              {copy.chrome.openApp}
            </a>
            <WalletConnectButton className="max-w-full flex-1 px-3 sm:flex-none" />
          </div>
        </div>

          <nav className="no-scrollbar flex min-w-0 items-center justify-between gap-1 overflow-x-auto border-t border-[#e7edf5] pt-3 pb-1 sm:justify-start sm:gap-1.5">
            {navItems.map((item) => {
              const mobilePrimary = item.href === "/products" || item.href === "/products/record-verification" || item.href === "/auctions";
              return (
              <Link
                className={cn(
                  buttonVariants({ size: "sm", variant: "ghost" }),
                  mobilePrimary ? "inline-flex" : "hidden sm:inline-flex",
                  "shrink-0 px-1.5 text-[11px] text-[#425570] sm:px-3 sm:text-sm",
                )}
                href={item.href}
                key={item.href}
              >
                {resolvePrimaryLabel(item.href, item.label)}
              </Link>
              );
            })}
        </nav>

        <div className="hidden border-t border-[#e7edf5] pt-3 lg:flex lg:flex-row lg:items-center lg:justify-between lg:gap-3">
          <div className="flex min-w-0 items-center gap-2 rounded-full border border-[#dce5f0] bg-[#f7f9fc] px-4 py-2 text-sm text-[#5d6d82]">
            <Search className="h-4 w-4 text-[#175cd3]" />
            <Link href="/search" className="truncate">
              {copy.chrome.searchSite}
            </Link>
                <span className="ml-auto hidden rounded-full border border-[#dce5f0] bg-white px-2 py-0.5 text-[11px] uppercase tracking-[0.18em] text-[#7a8ba0] sm:inline-flex">
              ⌘K
            </span>
          </div>

          <nav className="no-scrollbar flex items-center gap-1.5 overflow-x-auto pb-1">
            {utilityNav.map((item) => {
              return (
              <Link
                className={cn(buttonVariants({ size: "sm", variant: "ghost" }), "shrink-0 px-3 text-[#425570]")}
                href={item.href}
                key={item.href}
                rel={item.rel}
              >
                {resolveUtilityLabel(item.href, item.label)}
              </Link>
              );
            })}
          </nav>
        </div>
      </div>
    </header>
  );
}
