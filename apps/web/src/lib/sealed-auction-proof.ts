...
export type SealedAuctionProofPackage = {
  ...
  providerLanes: Array<{
    id: "commitment-integrity" | "magicblock-fast-session" | "room-access" | "public-verification";
    label: string;
    status: "verified";
...
export function buildSealedAuctionProofPackage(input: {
  ...
  const unsigned: SealedAuctionProofPackage = {
    ...
    providerLanes: [
      {
        id: "commitment-integrity",
        label: "Sealed-bid commitment integrity",
        status: "verified",
        publicCommitment: sha256Hex({ lane: "commitment-integrity", ...laneSeed }),
        verifierNote: "Bid amounts stay hidden during bidding. This lane is a hash commitment check, not a Groth16 proof.",
      },
      {
