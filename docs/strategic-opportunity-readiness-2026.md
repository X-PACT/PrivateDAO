...
- it only points to surfaces that are already live
- it distinguishes shipped strength from the next real gap
- it avoids claiming partner integration, audit completion, or closed funding that does not exist
...
- What is already shipped:
  ...
  - `/story`
  - `/services`
  - `/documents/custody-proof-reviewer-packet`
- Highest-value gap:
  - treasury professionalism and custody-proof continuity must stay extremely tight so the funding story remains believable.
- Best submission route:
  - `/start` -> `/story` -> `/services` -> `/documents/custody-proof-reviewer-packet`
...
- Highest-value gap:
  - continue tightening sender discipline, treasury references, and reviewer-visible payout evidence.
- Best submission route:
  - `/security` -> `/services` -> `/custody` -> `docs/grant-committee-pack.md`
...
- What is already shipped:
  ...
  - `/diagnostics`
- Highest-value gap:
  - external audit closure is still pending, so the entire corridor must stay evidence-bound and conservative.
- Best submission route:
  - `/documents/canonical-custody-proof` -> `/documents/authority-hardening-mainnet` -> `/documents/incident-readiness-runbook` -> `/diagnostics`

## Operating rule

PrivateDAO should keep using one product thesis:
...
Then adapt the corridor, not the product identity, for each adjacent opportunity.
