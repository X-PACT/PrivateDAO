...
const DEPLOYER_KEY = process.env.PDAO_EVM_DEPLOYER_PRIVATE_KEY;
...
async function main() {
  ...
  for (const network of networks) {
    ...
    const verifier = await deploy(wallet, publicClient, JSON.parse(await readFile(path.join(PACKAGE, "artifacts/Groth16Verifier.abi"), "utf8")), verifierBytecode);
    const blind = await deploy(wallet, publicClient, blindAbi, blindBytecode, [verifier.address]);
    const record = await deploy(wallet, publicClient, recordAbi, recordBytecode);
    ...
    results.push({ network: network.id, chainId: network.chainId, contracts: { verifier, blind, record }, record: { txHash: recordHash, blockNumber: recordReceipt.blockNumber.toString(), verificationId: recordVerificationId, explorerUrl: `${network.explorer}/tx/${recordHash}` }, blind: { txHash: blindHash, blockNumber: blindReceipt.blockNumber.toString(), verificationId: blindVerificationId, explorerUrl: `${network.explorer}/tx/${blindHash}` }, checks: { recordVerified: true, blindProofVerified: true, wrongChainRejected: true, alteredProofRejected: true }, links: { record: `https://privatedao.org/verify/evm/${network.id}/${recordVerificationId}`, blind: `https://privatedao.org/verify/evm/${network.id}/${blindVerificationId}` } });
  }

  const first = results[0]; const second = deployed[networks[1].id];
  const firstProof = deployed[networks[0].id];
  const secondPublicClient = createPublicClient({ chain: chainFor(networks[1]), transport: http(process.env[networks[1].rpcEnv], { timeout: 30_000 }) });
  const secondAccount = privateKeyToAccount(DEPLOYER_KEY);
  const secondRecordId = keccak256(toBytes(`${networks[0].id}:record-001`));
  const secondDomainBytes = encodeAbiParameters(domainTypes, ["PrivateDAO-Blind-Policy-v1", BigInt(networks[0].chainId), first.blind.address, PRODUCT_ID, SCHEMA_ID, secondRecordId]);
  ...
  await writeFile(path.join(PACKAGE, "deployments/phase-2-e2e.json"), JSON.stringify({ generatedAt: new Date().toISOString(), status: "testnet_verified", networks: results }, null, 2) + "\n");
  console.log(JSON.stringify({ status: "testnet_verified", networks: results }, null, 2));
}
