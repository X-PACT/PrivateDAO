...
`PRODUCT_CATALOG` is the single customer-executable capability surface for the current release. It intentionally lists only Solana Devnet until another network has a real provider, transaction lifecycle, receipt path, independent test evidence, and an application binding. Ethereum Sepolia now has independent Blind/Record E2E evidence in `packages/evm-verification`, but it is not exposed by this catalog until that binding is completed.
