...
const canonical_custody_proof_1 = require("../apps/web/src/lib/canonical-custody-proof");
function line(label, value) {
    return `- ${label}: \`${value ?? "pending"}\``;
}
function main() {
    const snapshot = (0, canonical_custody_proof_1.getCanonicalCustodyProofSnapshot)();
    const payload = {
        project: "PrivateDAO",
        generatedAt: new Date().toISOString(),
        ...snapshot,
    };
    fs_1.default.writeFileSync(path_1.default.resolve("docs/canonical-custody-proof.generated.json"), `${JSON.stringify(payload, null, 2)}\n`);
    fs_1.default.writeFileSync(path_1.default.resolve("docs/canonical-custody-proof.generated.md"), buildMarkdown(payload));
...
${payload.signers
        .map((signer) => `### Slot ${signer.slot} · ${signer.role}
