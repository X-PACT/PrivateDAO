...
const DEFAULT_PRIMARY_RPC = process.env.ANCHOR_PROVIDER_URL ||
...
async function main() {
    ...
    const primary = new web3_js_1.Connection(DEFAULT_PRIMARY_RPC, "confirmed");
    const fallback = new web3_js_1.Connection(DEFAULT_FALLBACK_RPC, "confirmed");
...
${anchorChecks
        .map((entry) => `- ${entry.label}: \`${entry.address}\` | exists: ${entry.exists ? "yes" : "no"} | owner: \`${entry.owner || "n/a"}\` | lamports: ${entry.lamports ?? 0} | data length: ${entry.dataLength ?? 0}`)
        .join("\n")}
...
`;
    ...
    fs_1.default.writeFileSync(path_1.default.resolve("docs/devnet-canary.generated.md"), md, "utf8");
    console.log("Wrote devnet canary report");
}
async function measureRpc(label, url, connection) {
...
async function inspectAccount(connection, label, address) {
    const info = await connection.getAccountInfo(new web3_js_1.PublicKey(address), "confirmed");
    return {
        label,
        address,
        exists: Boolean(info),
...
async function readTokenSupply(connection, mint) {
    try {
        return await connection.getTokenSupply(new web3_js_1.PublicKey(mint));
    }
    catch {
        return null;
    }
