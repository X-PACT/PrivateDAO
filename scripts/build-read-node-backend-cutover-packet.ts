...
function main() {
  ...
  const payload: ReadNodeBackendCutoverPacket = {
    ...
    activationState:
      "Indexed governance reads, runtime evidence, and the hosted read-node API are live. The canonical frontend remains https://privatedao.org/ while the API is served from https://api.privatedao.org/api/v1.",
    exactBoundary:
      "Use https://api.privatedao.org/api/v1 as the live read-node boundary. Same-domain `/api/v1` on the static frontend remains a future reverse-proxy consolidation step, not the current production-candidate API host.",
    deploymentTarget: {
