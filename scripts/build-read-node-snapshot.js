...
const JSON_PATH = path_1.default.join(ROOT, "docs/read-node/snapshot.generated.json");
const MD_PATH = path_1.default.join(ROOT, "docs/read-node/snapshot.generated.md");
const FRONTEND_TS_PATH = path_1.default.join(ROOT, "apps/web/src/lib/read-node-proposal-context.generated.ts");
...
function readJson(filePath) {
    return JSON.parse(fs_1.default.readFileSync(path_1.default.join(ROOT, filePath), "utf8"));
}
function formatLamportsToSol(lamports) {
    return Number((lamports / 1000000000).toFixed(4));
}
function formatRawUnits(value) {
    return new Intl.NumberFormat("en-US").format(value);
}
function formatTimelockLabel(seconds) {
...
function formatSourceLabel(title, phase) {
    return `Backend-indexed proposal record: ${title} (${phase})`;
}
function proposalRegistryId(proposal) {
    if (proposal.pubkey === FEATURED_PROPOSAL_KEYS.payroll)
        return "PDAO-104";
    if (proposal.pubkey === FEATURED_PROPOSAL_KEYS.gaming)
        return "PDAO-105";
    if (proposal.pubkey === FEATURED_PROPOSAL_KEYS.grant)
        return "PDAO-106";
...
function mapIndexedPhaseToPresentationStatus(phase, options) {
    if (phase === "Executed") {
        return "Executed";
    }
    if (phase === "Executable") {
...
function buildWindowSummary(phase, status) {
    if (status === "Executed") {
...
function buildTreasurySummary(proposal, status) {
    ...
    if (proposal.confidentialPayoutPlan) {
        ...
        const mint = proposal.confidentialPayoutPlan.tokenMint
            ...
            : "native asset";
        const statusLead = status === "Executed"
...
function buildQuorumSummary(proposal) {
...
function buildPrivacySummary(proposal) {
...
function buildTechSummary(proposal) {
    const tech = new Set();
...
function buildRegistrySummary(proposal, status) {
...
function inferProposalType(proposal) {
...
function inferExecutionTarget(proposal) {
    if (proposal.magicblockCorridor) {
...
function buildProposalContext(proposal, reportEntry, integrations) {
    ...
    const magicBlockExecute = proposal.pubkey === FEATURED_PROPOSAL_KEYS.payroll
    ...
    const recipient = treasuryAction?.recipient ??
    ...
    return {
        ...
        mintSymbol,
        mintAddress,
        timelockHours: typeof proposal.daoDetails?.executionDelaySeconds === "number"
        ...
        repeatedAttempts: Math.max(0, (reportEntry?.commitTxs?.length ?? 0) +
...
function buildFeaturedProposalContexts(proposals, report, integrations) {
...
function buildProposalRegistry(proposals, report, integrations) {
    const reportByPubkey = new Map((report.proposals ?? []).map((proposal) => [proposal.proposalPublicKey, proposal]));
...
async function main() {
    ...
    const report = readJson("docs/devnet-multi-proposal-report.json");
    const integrations = readJson("docs/frontier-integrations.generated.json");
    ...
    const featuredProposalRegistry = proposalRegistry.filter((proposal) => [FEATURED_PROPOSAL_KEYS.payroll, FEATURED_PROPOSAL_KEYS.gaming, FEATURED_PROPOSAL_KEYS.grant].includes(proposal.execution.proposalAccount));
    const payload = {
        generatedAt: new Date().toISOString(),
    ...
    fs_1.default.writeFileSync(MD_PATH, `# Read Node Snapshot
...
`;
    fs_1.default.writeFileSync(FRONTEND_TS_PATH, frontendFile);
    console.log(`Wrote read-node snapshot: ${path_1.default.relative(process.cwd(), JSON_PATH)}`);
}
main().catch((error) => {
