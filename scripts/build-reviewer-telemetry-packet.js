...
function finalizedCount(entries) {
...
function main() {
    ...
    const frontierIntegrations = readJson("docs/frontier-integrations.generated.json");
    const readNodeSnapshot = readJson("docs/read-node/snapshot.generated.json");
    const repoRoot = process.cwd();
    process.chdir(path_1.default.resolve(repoRoot, "apps/web"));
    // Load the metrics module from the app so this packet stays synced to the same summaries used by the UI.
    const metricsModule = require(path_1.default.resolve(repoRoot, "apps/web/src/lib/devnet-service-metrics"));
    ...
    const zkAnchorsConfirmed = frontierIntegrations.zk.anchorChecks.filter((entry) => entry.confirmed && entry.account.exists).length;
    const exportReadySummaries = [
    ...
    const payload = {
        ...
        reviewerIntent: "Show the shortest truth-synced route into runtime maturity, hosted reads, indexed governance, and infrastructure-facing reviewer value without claiming unsupported partnerships or mainnet readiness.",
        truthSources: [
            {
    ...
    fs_1.default.writeFileSync(path_1.default.resolve("docs/reviewer-telemetry-packet.generated.json"), `${JSON.stringify(payload, null, 2)}\n`);
    fs_1.default.writeFileSync(path_1.default.resolve("docs/reviewer-telemetry-packet.generated.md"), buildMarkdown(payload));
...
${payload.exportReadySummaries
        .map((entry) => `- ${entry.label}: ${entry.value} — ${entry.detail} (${entry.routeLabel}: ${entry.routeHref})`)
        .join("\n")}
