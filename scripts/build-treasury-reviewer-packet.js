...
function resolveReceiveAddress(symbol) {
    if (symbol === "SOL") {
        return (envValue(process.env.NEXT_PUBLIC_TREASURY_SOL_RECEIVE_ADDRESS) ??
            envValue(process.env.NEXT_PUBLIC_TREASURY_RECEIVE_ADDRESS) ??
            DEFAULT_TESTNET_TREASURY);
    }
    if (symbol === "USDC") {
        return (envValue(process.env.NEXT_PUBLIC_TREASURY_USDC_RECEIVE_ADDRESS) ??
            envValue(process.env.NEXT_PUBLIC_TREASURY_RECEIVE_ADDRESS) ??
            DEFAULT_TESTNET_TREASURY);
    }
    return (envValue(process.env.NEXT_PUBLIC_TREASURY_USDG_RECEIVE_ADDRESS) ??
        envValue(process.env.NEXT_PUBLIC_TREASURY_RECEIVE_ADDRESS) ??
        DEFAULT_TESTNET_TREASURY);
}
...
function buildExplorerHref(address, network) {
...
function main() {
    ...
    const ecosystem = readJson("docs/ecosystem-focus-alignment.generated.json");
    const launchTrust = readJson("docs/launch-trust-packet.generated.json");
    const treasury = getTreasuryReceiveConfig();
    const alignment = ecosystem.focusAreas.filter((item) => ["payments", "dao-tooling", "developer-tooling"].includes(item.slug));
    const packet = {
        project: "PrivateDAO",
        generatedAt: new Date().toISOString(),
        reviewerIntent: "Explain the treasury intake and payout posture as a reviewer-grade infrastructure surface, with strict sender discipline, public rails, proof links, commercial fit, and exact blocker visibility.",
    ...
    fs_1.default.writeFileSync(path_1.default.resolve("docs/treasury-reviewer-packet.generated.json"), `${JSON.stringify(packet, null, 2)}\n`);
    fs_1.default.writeFileSync(path_1.default.resolve("docs/treasury-reviewer-packet.generated.md"), buildMarkdown(packet));
