...
exports.resolveLoadProfile = resolveLoadProfile;
exports.createConnection = createConnection;
exports.loadOrInitializeState = loadOrInitializeState;
exports.persistState = persistState;
exports.runWalletGeneration = runWalletGeneration;
exports.runFundingPhase = runFundingPhase;
exports.runBootstrapPhase = runBootstrapPhase;
exports.runCommitPhase = runCommitPhase;
exports.runRevealPhase = runRevealPhase;
exports.runExecutePhase = runExecutePhase;
exports.runZkPhase = runZkPhase;
exports.runAdversarialPhase = runAdversarialPhase;
exports.publishArtifacts = publishArtifacts;
exports.runReportPhase = runReportPhase;
exports.runAllPhases = runAllPhases;
...
const FUNDING_CONFIRM_TIMEOUT_MS = Number(process.env.PRIVATE_DAO_FUNDING_CONFIRM_TIMEOUT_MS || 18000);
const FUNDING_STATUS_POLL_MS = 1200;
const TARGET_SOL_LAMPORTS = Math.floor(0.08 * web3_js_1.LAMPORTS_PER_SOL);
const COORDINATOR_MIN_BALANCE_LAMPORTS = Math.floor(8 * web3_js_1.LAMPORTS_PER_SOL);
...
const TREASURY_DEPOSIT_LAMPORTS = Math.floor(0.3 * web3_js_1.LAMPORTS_PER_SOL);
const TREASURY_TRANSFER_LAMPORTS = Math.floor(0.05 * web3_js_1.LAMPORTS_PER_SOL);
...
const ZK_DEVNET_DIR = path_1.default.join(REPO_ROOT, "zk", "devnet-load");
const ZK_LAYER_SEEDS = {
    vote: 1,
    delegation: 2,
    tally: 3,
};
...
const ZK_VERIFICATION_KEY_PATHS = {
...
async function confirmSignatureWithTimeout(connection, signature, timeoutMs) {
...
async function sendAndConfirmFundingTransaction(connection, tx, signers) {
    const payer = signers[0];
    if (!payer)
        throw new Error("funding transaction requires at least one signer");
...
function saveKeypair(filePath, keypair) {
    ensureDir(path_1.default.dirname(filePath));
    fs_1.default.writeFileSync(filePath, JSON.stringify(Array.from(keypair.secretKey)) + "\n", "utf8");
}
function resolveLoadProfile(input) {
    const normalized = (input || process.env.PRIVATE_DAO_LOAD_PROFILE || DEFAULT_PROFILE);
...
function profileTargetPdaoRaw(profile) {
    return BigInt(profile.targetPdaoUi) * 1000000000n;
}
function profileArtifactSuffix(profile) {
    return profile.name === DEFAULT_PROFILE ? "" : `.profile-${profile.name}`;
}
function resolveHarnessPaths(profile) {
    const suffix = profileArtifactSuffix(profile);
    const walletsDir = profile.name === DEFAULT_PROFILE
        ? path_1.default.join(REPO_ROOT, "scripts", "generated-wallets")
        : path_1.default.join(REPO_ROOT, "scripts", "generated-wallets", `profile-${profile.name}`);
    return {
        walletsDir,
        statePath: profile.name === DEFAULT_PROFILE
            ? path_1.default.join(walletsDir, "load-test-state.json")
            : path_1.default.join(walletsDir, `load-test-state.profile-${profile.name}.json`),
        docsWalletRegistry: path_1.default.join(REPO_ROOT, "docs", `devnet-wallet-registry${suffix}.json`),
        docsBootstrap: path_1.default.join(REPO_ROOT, "docs", `devnet-bootstrap${suffix}.json`),
        docsTxRegistry: path_1.default.join(REPO_ROOT, "docs", `devnet-tx-registry${suffix}.json`),
        docsAdversarial: path_1.default.join(REPO_ROOT, "docs", `adversarial-report${suffix}.json`),
        docsZkRegistry: path_1.default.join(REPO_ROOT, "docs", `zk-proof-registry${suffix}.json`),
        docsPerformance: path_1.default.join(REPO_ROOT, "docs", `performance-metrics${suffix}.json`),
        docsLoadReport: path_1.default.join(REPO_ROOT, "docs", `load-test-report${suffix}.md`),
    };
}
function walletPathForIndex(index, profile) {
    const width = Math.max(2, String(profile.walletCount).length);
    return path_1.default.join(resolveHarnessPaths(profile).walletsDir, `wallet-${String(index).padStart(width, "0")}.json`);
}
function walletRoleForIndex(index, profile) {
    ...
    if (slot < voterSlots)
        return "voter";
    if (slot < voterSlots + adversarialSlots)
        return "adversarial";
    return "zk-tester";
}
function walletPlanForIndex(index, role, profile) {
    const slot = index % profile.waveSize;
    if (role === "adversarial") {
        const alternating = slot < Math.max(1, Math.floor(profile.waveSize * 0.8))
...
function createEmptyWalletState(index, keypairPath, publicKey, profile) {
...
async function loadOrInitializeState(connection, profileInput) {
    var _a;
    ...
    const existing = readJsonIfExists(paths.statePath);
    if (existing) {
        existing.profile || (existing.profile = profile);
        existing.onchainZkAnchors || (existing.onchainZkAnchors = []);
        ...
        (_a = existing.coordinator).initialBalanceLamports || (_a.initialBalanceLamports = initialBalanceLamports);
        return existing;
    }
    const wallets = [];
    for (let index = 0; index < profile.walletCount; index += 1) {
        const keypairPath = walletPathForIndex(index + 1, profile);
        const keypair = fs_1.default.existsSync(keypairPath) ? loadKeypair(keypairPath) : web3_js_1.Keypair.generate();
...
function phaseTiming(state, phase) {
...
function coordinatorKeypair() {
    return loadKeypair(getCoordinatorWalletPath());
}
async function withRetry(state, label, fn, maxAttempts = 3) {
    let attempt = 0;
    let lastError;
...
function pushTxRecord(state, entry) {
    const alreadyRecorded = state.txRegistry.some((candidate) => candidate.txSignature === entry.txSignature &&
...
function pushAdversarial(state, entry) {
...
async function ensureCoordinatorFunding(state, connection) {
    ...
    while (remaining > 0) {
        const chunk = Math.min(remaining, 2 * web3_js_1.LAMPORTS_PER_SOL);
...
async function coordinatorPdaoSource(connection) {
    const coordinator = coordinatorKeypair();
    const account = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, coordinator, PDAO_MINT, coordinator.publicKey, false, COMMITMENT, undefined, spl_token_1.TOKEN_2022_PROGRAM_ID);
    return account.address;
}
async function ensureWalletFunding(state, connection, wallet) {
    const coordinator = coordinatorKeypair();
    const walletKeypair = loadKeypair(wallet.keypairPath);
    const sourceAta = await coordinatorPdaoSource(connection);
    const mintInfo = await (0, spl_token_1.getMint)(connection, PDAO_MINT, COMMITMENT, spl_token_1.TOKEN_2022_PROGRAM_ID);
    const recipientAta = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, coordinator, PDAO_MINT, walletKeypair.publicKey, false, COMMITMENT, undefined, spl_token_1.TOKEN_2022_PROGRAM_ID);
    ...
    if (wallet.funding.solBalanceLamports < wallet.funding.solTargetLamports) {
        ...
        const { result: signature, retries } = await withRetry(state, `fund-sol-${wallet.walletIndex}`, async () => sendAndConfirmFundingTransaction(connection, tx, [coordinator]));
    ...
    const recipientAccount = await (0, spl_token_1.getAccount)(connection, recipientAta.address, COMMITMENT, spl_token_1.TOKEN_2022_PROGRAM_ID);
    ...
    if (currentRaw < targetPdaoRaw) {
        ...
        const tx = new web3_js_1.Transaction().add((0, spl_token_1.createTransferCheckedInstruction)(sourceAta, PDAO_MINT, recipientAta.address, coordinator.publicKey, diff, mintInfo.decimals, [], spl_token_1.TOKEN_2022_PROGRAM_ID));
        const started = Date.now();
        const { result: signature, retries } = await withRetry(state, `fund-pdao-${wallet.walletIndex}`, async () => sendAndConfirmFundingTransaction(connection, tx, [coordinator]));
    ...
    wallet.funding.solBalanceLamports = await connection.getBalance(walletKeypair.publicKey, COMMITMENT);
    wallet.funding.pdaoBalanceRaw = (await (0, spl_token_1.getAccount)(connection, recipientAta.address, COMMITMENT, spl_token_1.TOKEN_2022_PROGRAM_ID)).amount.toString();
...
async function runWalletGeneration(connection, profileInput) {
...
async function runFundingPhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
    ...
    for (let i = 0; i < state.wallets.length; i += state.profile.fundingWaveSize) {
        ...
        for (const wallet of wave) {
            try {
                const { retries } = await withRetry(state, `fund-wallet-${wallet.walletIndex}`, async () => ensureWalletFunding(state, connection, wallet));
                wallet.funding.retries += retries;
                persistState(state);
                publishArtifacts(state);
            }
...
async function fetchProposalAndDao(connection, wallet, proposalPk) {
...
async function runBootstrapPhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
    ...
    if (state.bootstrap) {
        try {
            ...
            await program.account.dao.fetch(new web3_js_1.PublicKey(state.bootstrap.daoPublicKey));
            await program.account.proposal.fetch(new web3_js_1.PublicKey(state.bootstrap.proposalPublicKey));
    ...
    const recipient = new web3_js_1.PublicKey(state.wallets[0].publicKey);
    const [daoPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("dao"), coordinator.publicKey.toBuffer(), Buffer.from(daoName)], PROGRAM_ID);
    const createDaoStarted = Date.now();
    const { result: createDaoTx, retries: createDaoRetries } = await withRetry(state, "create-dao", async () => program.methods
        .initializeDao(daoName, DAO_QUORUM_PERCENT, new anchor_1.BN(0), new anchor_1.BN(REVEAL_WINDOW_SECONDS), new anchor_1.BN(EXECUTION_DELAY_SECONDS), { tokenWeighted: {} })
        .accounts({
        dao: daoPda,
    ...
    const { result: depositTx, retries: depositRetries } = await withRetry(state, "deposit-treasury", async () => program.methods
        .depositTreasury(new anchor_1.BN(TREASURY_DEPOSIT_LAMPORTS))
        .accounts({
        dao: daoPda,
    ...
    const { result: createProposalTx, retries: proposalRetries } = await withRetry(state, "create-proposal", async () => program.methods
        .createProposal(proposalTitle, "Wave-based Devnet stress test covering commit, reveal, finalize, execute, zk, and adversarial flows.", new anchor_1.BN(PROPOSAL_DURATION_SECONDS), {
        actionType: { sendSol: {} },
        amountLamports: new anchor_1.BN(TREASURY_TRANSFER_LAMPORTS),
        recipient,
        tokenMint: null,
    })
        .accounts({
    ...
    state.bootstrap = {
        ...
        daoExplorer: (0, utils_1.solscanAccountUrl)(daoPda.toBase58()),
        treasuryPda: treasuryPda.toBase58(),
        treasuryExplorer: (0, utils_1.solscanAccountUrl)(treasuryPda.toBase58()),
        proposalPublicKey: proposalPda.toBase58(),
        proposalExplorer: (0, utils_1.solscanAccountUrl)(proposalPda.toBase58()),
...
function ensureBootstrap(state) {
    if (!state.bootstrap) {
        throw new Error("bootstrap phase has not completed");
    }
    return state.bootstrap;
}
async function recordValidCommit(state, connection, wallet, wave) {
    ...
    const proposalPda = new web3_js_1.PublicKey(bootstrap.proposalPublicKey);
    const proposal = await program.account.proposal.fetch(proposalPda);
    ...
    const saltHex = wallet.internal?.saltHex ?? crypto_1.default.randomBytes(32).toString("hex");
    const salt = Buffer.from(saltHex, "hex");
    const commitment = (0, utils_1.computeProposalCommitment)(wallet.plan.vote, salt, keypair.publicKey, proposalPda);
    ...
    persistState(state);
    const started = Date.now();
    const { result: txSignature, retries } = await withRetry(state, `commit-${wallet.walletIndex}`, async () => program.methods
...
async function expectCommitFailure(state, connection, wallet, wave) {
    ...
    const commitment = (0, utils_1.computeProposalCommitment)(false, salt, keypair.publicKey, proposalPda);
    const nextWallet = state.wallets[(wallet.walletIndex % state.wallets.length)];
    const wrongPubkey = new web3_js_1.PublicKey(nextWallet.publicKey);
    const scenario = wallet.plan.commitScenario;
    const recordFailure = (error, note) => {
    ...
    try {
        if (scenario === "delegate-then-direct-overlap") {
            ...
            const { result: delegationTx, retries: delegationRetries } = await withRetry(state, `delegate-${wallet.walletIndex}`, async () => program.methods
        ...
        const wrongVoterRecord = scenario === "wrong-voter-record"
            ? voteRecordPdaFor(proposalPda, wrongPubkey)
            : voteRecordPdaFor(proposalPda, keypair.publicKey);
        const wrongDelegationMarker = scenario === "wrong-delegation-marker"
            ? delegationPdaFor(proposalPda, wrongPubkey)
            : delegationPdaFor(proposalPda, keypair.publicKey);
        const wrongTokenAccount = scenario === "wrong-token-account"
            ? (0, spl_token_1.getAssociatedTokenAddressSync)(PDAO_MINT, coordinatorKeypair().publicKey, false, spl_token_1.TOKEN_2022_PROGRAM_ID)
            : voterAta;
        await program.methods
            .commitVote([...commitment], null)
            .accounts({
...
async function runCommitPhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
...
async function expectRevealFailure(state, connection, actorWallet, targetWallet, scenario) {
    const bootstrap = ensureBootstrap(state);
    const actorKeypair = loadKeypair(actorWallet.keypairPath);
    const program = createProgram(connection, actorKeypair);
    const proposalPda = new web3_js_1.PublicKey(bootstrap.proposalPublicKey);
    const voterRecord = voteRecordPdaFor(proposalPda, new web3_js_1.PublicKey(targetWallet.publicKey));
    ...
    const salt = Buffer.from(saltHex, "hex");
    const revealSalt = scenario === "wrong-salt"
        ? Buffer.from(crypto_1.default.createHash("sha256").update(salt).digest("hex").slice(0, 64), "hex")
        : salt;
    try {
        await program.methods
...
async function recordValidReveal(state, connection, wallet, wave) {
    ...
    const voterRecord = voteRecordPdaFor(proposalPda, keypair.publicKey);
    const started = Date.now();
    const { result: txSignature, retries } = await withRetry(state, `reveal-${wallet.walletIndex}`, async () => program.methods
...
async function runRevealPhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
    const bootstrap = ensureBootstrap(state);
    ...
    const proposal = await coordinatorProgram.account.proposal.fetch(new web3_js_1.PublicKey(bootstrap.proposalPublicKey));
    ...
    for (const [waveIndex, wave] of waves.entries()) {
        ...
        const waveAdversary = state.wallets.find((wallet) => Math.floor((wallet.walletIndex - 1) / state.profile.waveSize) === waveIndex && wallet.role === "adversarial");
    ...
    const refreshedProposal = await coordinatorProgram.account.proposal.fetch(new web3_js_1.PublicKey(bootstrap.proposalPublicKey));
    if (Math.floor(Date.now() / 1000) < refreshedProposal.revealEnd.toNumber()) {
        try {
            await coordinatorProgram.methods
                .finalizeProposal()
                .accounts({
                dao: new web3_js_1.PublicKey(bootstrap.daoPublicKey),
                proposal: new web3_js_1.PublicKey(bootstrap.proposalPublicKey),
...
async function runExecutePhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
    const bootstrap = ensureBootstrap(state);
    phaseTiming(state, "execute");
    const coordinator = coordinatorKeypair();
    const program = createProgram(connection, coordinator);
    const daoPda = new web3_js_1.PublicKey(bootstrap.daoPublicKey);
    const proposalPda = new web3_js_1.PublicKey(bootstrap.proposalPublicKey);
    const treasuryPda = new web3_js_1.PublicKey(bootstrap.treasuryPda);
    const executeArtifactsReady = state.adversarial.some((entry) => entry.id === "duplicate-finalize") &&
    ...
    if ((0, utils_1.proposalStatusLabel)(proposalBeforeFinalize.status) === "Voting") {
        const finalizeStarted = Date.now();
        const { result: finalizeTx, retries: finalizeRetries } = await withRetry(state, "finalize-valid", async () => program.methods
            .finalizeProposal()
            .accounts({
            dao: daoPda,
            proposal: proposalPda,
    ...
    if (!state.adversarial.some((entry) => entry.id === "duplicate-finalize")) {
        try {
            ...
            pushAdversarial(state, {
                ...
                invariantState: {
                    statusBefore: (0, utils_1.proposalStatusLabel)(finalizedProposal.status),
                },
            });
        }
        catch (error) {
            pushAdversarial(state, {
                ...
                invariantState: {
                    statusBefore: (0, utils_1.proposalStatusLabel)(finalizedProposal.status),
                    statusAfter: (0, utils_1.proposalStatusLabel)((await program.account.proposal.fetch(proposalPda)).status),
                },
            });
            state.metrics.attemptCount += 1;
            state.metrics.failureCount += 1;
    ...
    if (!finalizedProposal.isExecuted && !state.adversarial.some((entry) => entry.id === "execute-before-unlock")) {
        try {
            await program.methods
                ...
                proposal: proposalPda,
                treasury: treasuryPda,
                treasuryRecipient: new web3_js_1.PublicKey(bootstrap.proposalRecipient),
    ...
    if (!finalizedProposal.isExecuted) {
        ...
        const { result: executeTx, retries: executeRetries } = await withRetry(state, "execute-valid", async () => program.methods
            .executeProposal()
            .accounts({
            dao: daoPda,
            proposal: proposalPda,
    ...
    pushAdversarial(state, {
        ...
        error: null,
        invariantState: {
            statusBeforeFinalize: (0, utils_1.proposalStatusLabel)(proposalBeforeFinalize.status),
            statusAfterFinalize: (0, utils_1.proposalStatusLabel)(finalizedProposal.status),
            statusAfterExecute: (0, utils_1.proposalStatusLabel)((await program.account.proposal.fetch(proposalPda)).status),
        },
    });
    completePhase(state, "execute");
    persistState(state);
...
function fieldId(input) {
    const digest = crypto_1.default.createHash("sha256").update(input).digest("hex");
...
function hashHexBuffers(...values) {
...
function zkProofAnchorPdaFor(proposal, layer) {
    const seedByte = ZK_LAYER_SEEDS[layer];
    if (!seedByte)
        throw new Error(`unsupported zk proof layer for anchor: ${layer}`);
    return web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("zk-proof"), proposal.toBuffer(), Buffer.from([seedByte])], PROGRAM_ID)[0];
}
...
async function generateVoteProof(state, wallet, weightRaw) {
    const bootstrap = ensureBootstrap(state);
    ...
    const inputPath = path_1.default.join(ZK_DEVNET_DIR, `${baseName}.input.json`);
    const witnessPath = path_1.default.join(ZK_DEVNET_DIR, `${baseName}.wtns`);
    const proofPath = path_1.default.join(ZK_DEVNET_DIR, `${baseName}.proof.json`);
    const publicPath = path_1.default.join(ZK_DEVNET_DIR, `${baseName}.public.json`);
    writeJson(inputPath, {
    ...
    return {
        ...
        verified: true,
        generatedAt: nowIso(),
        inputPath: path_1.default.relative(REPO_ROOT, inputPath),
        proofPath: path_1.default.relative(REPO_ROOT, proofPath),
        publicPath: path_1.default.relative(REPO_ROOT, publicPath),
    };
}
async function generateTallyProof(state, wallets) {
    if (wallets.length < 2) {
    ...
    const commitments = await Promise.all(votes.map((vote, index) => poseidonHash(vote, salts[index], voterKeys[index], proposalId, daoKey)));
    ...
    const inputPath = path_1.default.join(ZK_DEVNET_DIR, "tally.input.json");
    const witnessPath = path_1.default.join(ZK_DEVNET_DIR, "tally.wtns");
    const proofPath = path_1.default.join(ZK_DEVNET_DIR, "tally.proof.json");
    const publicPath = path_1.default.join(ZK_DEVNET_DIR, "tally.public.json");
    writeJson(inputPath, {
        proposalId: proposalId.toString(),
        daoKey: daoKey.toString(),
...
async function generateDelegationProof(state, wallet) {
    const bootstrap = ensureBootstrap(state);
    const proposalId = BigInt(bootstrap.proposalId);
    const daoKey = fieldId(bootstrap.daoPublicKey);
    const delegatorKey = fieldId(wallet.publicKey);
    const delegateeKey = fieldId(state.coordinator.publicKey);
    const salt = BigInt(`0x${crypto_1.default.randomBytes(32).toString("hex")}`);
    ...
    const inputPath = path_1.default.join(ZK_DEVNET_DIR, `delegation-${wallet.walletIndex.toString().padStart(2, "0")}.input.json`);
    const witnessPath = path_1.default.join(ZK_DEVNET_DIR, `delegation-${wallet.walletIndex.toString().padStart(2, "0")}.wtns`);
    const proofPath = path_1.default.join(ZK_DEVNET_DIR, `delegation-${wallet.walletIndex.toString().padStart(2, "0")}.proof.json`);
    const publicPath = path_1.default.join(ZK_DEVNET_DIR, `delegation-${wallet.walletIndex.toString().padStart(2, "0")}.public.json`);
    writeJson(inputPath, {
        proposalId: proposalId.toString(),
        daoKey: daoKey.toString(),
        minWeight: "1",
...
async function anchorZkProofEntry(state, connection, program, recorder, entry) {
    const bootstrap = ensureBootstrap(state);
    const dao = new web3_js_1.PublicKey(bootstrap.daoPublicKey);
    const proposal = new web3_js_1.PublicKey(bootstrap.proposalPublicKey);
    ...
    if (existing)
        return existing;
    const { result: existingAccount, retries: anchorReadRetries } = await withRetry(state, `zk-anchor-read:${entry.layer}`, () => connection.getAccountInfo(anchorPda, COMMITMENT), 5);
    state.metrics.retryCount += anchorReadRetries;
    if (existingAccount) {
        throw new Error(`zk proof anchor already exists for ${entry.layer} (${anchorPda.toBase58()}) but no tx signature is recorded in state`);
    }
    const verificationKeyPath = ZK_VERIFICATION_KEY_PATHS[entry.circuit];
    if (!verificationKeyPath)
        throw new Error(`unsupported zk circuit for anchor: ${entry.circuit}`);
    const verificationKeyHash = fileSha256(path_1.default.join(REPO_ROOT, verificationKeyPath));
    const bundleHash = hashHexBuffers(entry.proofHash, entry.publicInputsHash, verificationKeyHash);
    const transaction = await program.methods
        .anchorZkProof(ZK_LAYER_ARGS[entry.layer], { groth16: {} }, [...Buffer.from(entry.proofHash, "hex")], [...Buffer.from(entry.publicInputsHash, "hex")], [...Buffer.from(verificationKeyHash, "hex")], [...bundleHash])
        .accounts({
        dao,
        proposal,
...
async function anchorRepresentativeZkProofs(state, connection, entries) {
    ...
    for (const layer of ["vote", "delegation", "tally"]) {
        ...
        if (!entry)
            throw new Error(`missing generated zk proof entry for ${layer}`);
...
async function runZkPhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
    ensureBootstrap(state);
    ...
    const entries = state.zkProofs.length >= 7 ? state.zkProofs : [];
    if (entries.length < 7) {
...
async function runAdversarialPhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
...
function buildPerformanceMetrics(state) {
    ...
    const averageTxLatencyMs = latencies.length > 0 ? latencies.reduce((sum, value) => sum + value, 0) / latencies.length : 0;
...
function buildWalletRegistryDoc(state) {
    return {
        generatedAt: nowIso(),
        runLabel: state.runLabel,
        network: state.network,
...
function buildBootstrapDoc(state) {
    if (!state.bootstrap)
        return null;
    return {
        generatedAt: nowIso(),
...
function buildTxRegistryDoc(state) {
    return {
        generatedAt: nowIso(),
        runLabel: state.runLabel,
        network: state.network,
...
function buildAdversarialDoc(state) {
    return {
        generatedAt: nowIso(),
        runLabel: state.runLabel,
        network: state.network,
...
function buildZkRegistryDoc(state) {
    return {
        generatedAt: nowIso(),
        runLabel: state.runLabel,
        network: state.network,
...
function buildLoadTestReport(state, metrics) {
    const bootstrap = ensureBootstrap(state);
    ...
    const invalidRevealRejections = state.adversarial.filter((entry) => entry.phase === "reveal" && entry.outcome === "rejected").length;
    ...
    const confidentialPayoutSuccess = state.txRegistry.filter((entry) => entry.action === "execute-confidential-payout-plan").length;
    const replayRejections = state.adversarial.filter((entry) => entry.outcome === "rejected" &&
        (entry.scenario === "replay" || entry.scenario === "duplicate-execute" || entry.scenario === "duplicate-finalize")).length;
    const authorityMismatchRejections = state.adversarial.filter((entry) => entry.outcome === "rejected" &&
        (entry.scenario === "treasury-miswiring" ||
            entry.scenario === "wrong-voter-record" ||
            entry.scenario === "wrong-delegation-marker" ||
            entry.scenario === "wrong-token-account")).length;
...
function publishArtifacts(state) {
    ...
    if (state.bootstrap) {
        fs_1.default.writeFileSync(paths.docsLoadReport, buildLoadTestReport(state, metrics), "utf8");
    }
}
async function runReportPhase(connection, profileInput) {
    const state = await loadOrInitializeState(connection, profileInput);
...
async function runAllPhases(connection, profileInput) {
