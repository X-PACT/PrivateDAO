...
BIDDER_FUNDING_SOL="${AUCTION_BIDDER_FUNDING_SOL:-0.01}"
ccp solana transfer "$ADDR_A" "$BIDDER_FUNDING_SOL" --from "$CREATOR_KEYPAIR" --url https://api.devnet.solana.com --allow-unfunded-recipient
ccp solana transfer "$ADDR_B" "$BIDDER_FUNDING_SOL" --from "$CREATOR_KEYPAIR" --url https://api.devnet.solana.com --allow-unfunded-recipient
