...
deployments.evidence.treasury = { requestedHash, approvedHash, executedHash, paymentId, status: "executed" };
...
const commitEnd = BigInt(now + 5);
const revealEnd = BigInt(now + 12);
...
const biddingEnd = BigInt(auctionNow + 5);
const auctionRevealEnd = BigInt(auctionNow + 12);
...
assert.equal(Number(auctionState[6]), 3, "auction was not settled");
