...
async function main() {
  ...
  if (!dao || !newAuthority) {
    console.error("Usage: npm run transfer:treasury-operator-authority -- --dao <DAO_PDA> --new-authority <NEW_AUTHORITY>");
    process.exit(1);
  }
  ...
  const before = await program.account.treasuryOperatorAuthority.fetch(treasuryOperatorAuthority);
  if (!before.authority.equals(authority)) {
    throw new Error(`wallet ${authority.toBase58()} is not current treasury operator authority ${before.authority.toBase58()}`);
  }

  const tx = await program.methods
    .transferTreasuryOperatorAuthority(newAuthorityPk)
    .accounts({
      dao: daoPk,
  ...
  const after = await program.account.treasuryOperatorAuthority.fetch(treasuryOperatorAuthority);
  if (!after.authority.equals(newAuthorityPk)) {
    throw new Error(`treasury operator readout mismatch: expected ${newAuthorityPk.toBase58()}, got ${after.authority.toBase58()}`);
  }
