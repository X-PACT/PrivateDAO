...
function readKey(args) {
    const keyHex = args.keyHex
        ? String(args.keyHex)
        : args.keyFile
            ? fs_1.default.readFileSync(path_1.default.resolve(String(args.keyFile)), "utf8")
    ...
    if (key.length !== 32)
        throw new Error("confidential manifest key must be 32 bytes");
    return key;
}
...
function main() {
    ...
    const encryptedPath = path_1.default.resolve(String(args.encrypted || DEFAULT_ENCRYPTED_PATH));
    const raw = fs_1.default.readFileSync(encryptedPath, "utf8");
    ...
    if (key) {
        ...
        const decipher = (0, crypto_1.createDecipheriv)("aes-256-gcm", key, Buffer.from(encrypted.ivBase64, "base64"));
        ...
        if (args.plaintext) {
            const expected = Buffer.from(canonicalize(JSON.parse(fs_1.default.readFileSync(path_1.default.resolve(String(args.plaintext)), "utf8"))), "utf8");
