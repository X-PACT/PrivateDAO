...
function main() {
  ...
  const checks: Array<[string, string, string]> = [
    ...
    [siteHeader, 'href: "/auctions"', "site header is missing the Auctions route"],
    [siteHeader, 'href: "/agents"', "site header is missing the Agents route"],
    ...
    [fs.readFileSync(path.resolve("apps/web/src/app/rooms/join/page.tsx"), "utf8"), "Enter a room without making it discoverable", "/rooms/join is missing the private invitation entry"],
    [fs.readFileSync(path.resolve("apps/web/src/lib/entry-flow-copy.ts"), "utf8"), "Create room and generate invite", "private room entry is missing the real invite creation action"],
    [fs.readFileSync(path.resolve("apps/web/src/lib/entry-flow-copy.ts"), "utf8"), "No influence before reveal", "private room entry is missing the no-influence voting boundary"],
    [fs.readFileSync(path.resolve("apps/web/src/components/private-room-entry.tsx"), "utf8"), "/rooms/join#invite=", "private room invite links must keep bearer codes out of server query logs"],
    [fs.readFileSync(path.resolve("apps/web/src/components/public-dao-directory.tsx"), "utf8"), "/api/v1/proposals", "public DAO directory is not connected to the live proposal read path"],
