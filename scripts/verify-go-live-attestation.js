...
function main() {
    const attestationPath = path_1.default.resolve("docs/go-live-attestation.generated.json");
    if (!fs_1.default.existsSync(attestationPath)) {
        throw new Error("missing go-live attestation");
    }
    const attestation = readJson("docs/go-live-attestation.generated.json");
    ...
    for (const blocker of [
        ...
        assert(attestation.blockers.some((entry) => entry.name === blocker[0] && entry.status === blocker[1]), `go-live attestation is missing blocker: ${blocker[0]}=${blocker[1]}`);
    }
    assert(attestation.decision === "blocked-pending-external-steps", "go-live attestation decision should remain blocked until external dependencies close");
