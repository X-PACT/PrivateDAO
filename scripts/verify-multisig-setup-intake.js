...
function main() {
    const jsonPath = path_1.default.resolve("docs/multisig-setup-intake.json");
    const markdownPath = path_1.default.resolve("docs/multisig-setup-intake.md");
    assertFile(jsonPath);
    assertFile(markdownPath);
    const intake = readJson(jsonPath);
    ...
    assert(intake.timelock.shorteningPolicy.includes("must-not-shorten"), "timelock shortening policy must reject undocumented shortening");
    assert(intake.signers.length === 3, "multisig intake must define exactly 3 signer slots");
    const signerSlots = new Set();
    ...
    if (intake.status === "complete" || intake.status === "ready-for-transfer") {
        ...
        assert(typeof intake.timelock.configuredHours === "number" && intake.timelock.configuredHours >= 48, "completed/ready intake requires configured timelock >= 48 hours");
    }
    if (intake.status === "complete") {
        ...
        for (const transfer of intake.authorityTransfers) {
            ...
            assertReference(transfer.postTransferReadoutReferenceUrl ?? "", `${transfer.surface} postTransferReadoutReferenceUrl`);
        }
    }
    ...
    console.log(`Multisig setup intake verification: PASS (${intake.status}, ${populatedSignerKeys}/3 signer keys populated)`);
}
function assertTransferSurfaces(intake) {
...
function assertBackupRules(intake) {
    ...
    assert(intake.backupProcedures.signerReplacementProcedureDocumented === true, "signer replacement procedure must be documented");
...
function assertNoSecretMaterial(serialized) {
    const forbidden = [
        "seed phrase",
        "mnemonic",
        "private key",
