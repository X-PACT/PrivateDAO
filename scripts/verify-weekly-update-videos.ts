...
function main() {
  ...
  for (const file of EXPECTED.filter((name) => name.endsWith(".mp4"))) {
    ...
    if (!probe.ok) {
      console.warn(`Weekly update video verification fallback for ${file}: ${probe.reason}`);
      continue;
    }
    const videoStream = (probe.payload.streams || []).find((stream) => stream.codec_type === "video");
...
function probeMedia(file: string): { ok: true; payload: Probe } | { ok: false; reason: string } {
  ...
  if (probe.status !== 0) {
    return { ok: false, reason: probe.error?.message || probe.stderr || probe.stdout || "ffprobe unavailable" };
  }
  return { ok: true, payload: JSON.parse(probe.stdout) as Probe };
}
