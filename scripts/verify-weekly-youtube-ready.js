...
const ROOT = path_1.default.resolve("docs/assets/weekly-youtube-ready");
...
function main() {
    for (const file of EXPECTED_VIDEOS) {
        ...
        assertExists(path_1.default.join(ROOT, `${stem}.srt`), `Missing subtitle file for ${file}`, 64);
        assertExists(path_1.default.join(ROOT, `${stem}-voice.txt`), `Missing voice script for ${file}`, 64);
        assertExists(path_1.default.join(ROOT, `${stem}-voice.wav`), `Missing generated voice WAV for ${file}`, 1024);
        assertExists(path_1.default.join(ROOT, `${stem}-intro.mp4`), `Missing intro clip for ${file}`, 1024);
        assertExists(path_1.default.join(ROOT, `${stem}-mid.mp4`), `Missing mid clip for ${file}`, 1024);
        assertExists(path_1.default.join(ROOT, `${stem}-outro.mp4`), `Missing outro clip for ${file}`, 1024);
        assertExists(path_1.default.join(ROOT, `${stem}-sequence.mp4`), `Missing sequence clip for ${file}`, 1024);
    }
    console.log("Weekly YouTube-ready video verification: PASS");
}
function hyphenStem(file) {
    return path_1.default.basename(file, ".mp4").replace(/ /g, "-");
}
function assertExists(full, message, minSize = 1024) {
