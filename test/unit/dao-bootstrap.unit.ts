...
describe("dao bootstrap unit coverage", () => {
  ...
  it("reports missing DAO and proposal accounts explicitly", async () => {
    ...
    await expectRejected(
      ...
      /DAO account was not found on (Testnet|Mainnet Beta)/,
    );

    await expectRejected(
      fetchProposalAccountDetails(connection as never, Keypair.generate().publicKey),
      /Proposal account was not found on (Testnet|Mainnet Beta)/,
    );
  });
  ...
  it("surfaces explicit cluster execution errors during signature polling", async () => {
    await expectRejected(
      awaitLiveSignatureOnCluster({
        connection: createConnection({
          signatureStatuses: [
      ...
      /(Testnet|Mainnet Beta) rejected the submitted transaction/,
    );
  });
});
